## Test task files
### Files included
Provided cookbooks, environment and logs in the appropriate folders. Also included vagrant file.
### Scheme
Vagrant VMs are executed on the Ubuntu linux machine. To access gost VM machine SSH to 213.184.245.66. To obtain username and password - check out e-mail. To run vagrant VM go to ~/vagrantProject folder. There is 3 VMs created: chef server (192.168.10.10 server.chef.local or chef), workstation (192.168.10.3 workstation.chef.local or workstation) and node (192.168.10.2 node.chef.local or node).
### Problems 
